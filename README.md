...
- 天气和位置/地址结果分别缓存；天气刷新间隔默认 30 分钟（可配置 5–180），定位刷新间隔默认 10 分钟（可配置 5–60）。
- 自动定位每次先读取当前坐标，再按坐标复用地址与天气缓存；移动到不同坐标区域后自动切换缓存，避免继续注入旧地点。
...
- 注入块固定顺序：当前时间 → 当前天气 → 当前位置 → 当前电量 → 设备信息。
- 当前位置仅显示地址、精度、定位源、地址源和容错信息；当前位置和天气块均不注入坐标，当前位置也不注入定位时间。
...
v1.1.0 自动测试进一步覆盖：自定义设备名称优先级、角色卡 ID 去重、多选匹配、非匹配拦截、Prompt Hook 活动角色卡识别，以及设置页 onLoad 读取角色卡列表。

v1.2.0 自动测试进一步覆盖：天气与位置缓存命中、手动强制刷新、网络请求计数、刷新间隔默认值与保存，以及“刷新间隔”设置节位于“预览与测试”之前。

v1.3.0 自动测试进一步覆盖：天气 Auto 选项渲染、Open-Meteo → MET Norway → wttr.in 调用顺序、前序供应商失败后的逐级容错，以及 Auto 结果缓存命中。
